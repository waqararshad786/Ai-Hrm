        ...
        <div className="p-5 space-y-5">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="bg-white rounded-lg border border-blue-100 p-4">
              ...
              <div className="space-y-2">
                {[['Actual Time', formatTime(record.approvedCheckIn)], ['Requested', formatTime(record.requestedCheckIn)], ['Status', record.checkInRequest?.approved === false ? 'Pending' : 'Approved']].map(([label, value]) => (
            ...
            <div className="bg-white rounded-lg border border-red-100 p-4">
              ...
              <div className="space-y-2">
                {[['Actual Time', formatTime(record.approvedCheckOut)], ['Requested', formatTime(record.requestedCheckOut)], ['Status', record.checkOutRequest?.approved === false ? 'Pending' : 'Approved']].map(([label, value]) => (
                  <div key={label} className="flex justify-between"><span className="text-xs text-gray-500">{label}</span><span className="text-xs font-medium text-gray-800">{value || '—'}</span></div>
                ))}
              </div>
            </div>
      ...
      <div className="max-w-6xl mx-auto px-6 py-6 space-y-5">
        ...
        {(pendingRequests.checkIn || pendingRequests.checkOut) && (
          <div className="flex items-center gap-3 bg-yellow-50 border border-yellow-200 rounded-lg px-4 py-3">
            ...
            <div><p className="text-sm font-medium text-yellow-800">Pending Approval</p><p className="text-xs text-yellow-600 mt-0.5">{pendingRequests.checkIn && 'Check-in request awaiting admin approval. '}{pendingRequests.checkOut && 'Check-out request awaiting admin approval.'}</p></div>
          </div>
        )}
            ...
            <div className="flex-1">
              ...
              <div className="flex flex-wrap items-center gap-6">
                {[['Check In', displayTimes.checkIn, pendingRequests.checkIn], ['Check Out', displayTimes.checkOut, pendingRequests.checkOut]].map(([label, time, pending]) => (
                  <div key={label}><p className="text-xs text-gray-400 mb-1">{label}</p><p className="text-xl font-semibold text-gray-900">{time}</p>{pending && <p className="text-xs text-amber-500 mt-0.5">Pending approval</p>}</div>
                ))}
                <div><p className="text-xs text-gray-400 mb-1">Status</p><Badge variant={todayStatus.variant}>{todayStatus.label}</Badge></div>
                {todayAttendance?.approvedCheckIn && todayAttendance?.approvedCheckOut && (<div><p className="text-xs text-gray-400 mb-1">Hours worked</p><p className="text-xl font-semibold text-gray-900">{todayAttendance.totalHours?.toFixed(1) || 0}h</p></div>)}
              </div>
            </div>

            <div className="flex flex-col gap-2 min-w-[160px]">
              {canCheckIn && !loading && (<button onClick={handleCheckIn} disabled={loading} className="flex items-center justify-center gap-2 py-2.5 px-4 bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-medium rounded-lg transition-colors disabled:opacity-50"><FaCheckCircle className="text-xs" /> Check In</button>)}
              {pendingRequests.checkIn && (<div className="py-2.5 px-4 bg-yellow-50 border border-yellow-200 rounded-lg text-center"><p className="text-xs font-medium text-yellow-700">Check-in pending</p></div>)}
              {canCheckOut && !loading && (<button onClick={handleCheckOut} disabled={loading} className="flex items-center justify-center gap-2 py-2.5 px-4 bg-red-600 hover:bg-red-700 text-white text-sm font-medium rounded-lg transition-colors disabled:opacity-50"><FaTimesCircle className="text-xs" /> Check Out</button>)}
              {pendingRequests.checkOut && (<div className="py-2.5 px-4 bg-yellow-50 border border-yellow-200 rounded-lg text-center"><p className="text-xs font-medium text-yellow-700">Check-out pending</p></div>)}
              {todayAttendance?.approvedCheckIn && todayAttendance?.approvedCheckOut && (<div className="py-2.5 px-4 bg-green-50 border border-green-200 rounded-lg text-center"><p className="text-xs font-medium text-green-700">Day complete · {todayAttendance.totalHours?.toFixed(1) || 0}h</p></div>)}
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-lg border border-gray-100 overflow-hidden">
          <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
            <div className="flex items-center gap-3"><div className="w-8 h-8 bg-indigo-50 rounded-lg flex items-center justify-center"><FaHistory className="text-indigo-500 text-sm" /></div><div><p className="text-sm font-semibold text-gray-800">Attendance History</p><p className="text-xs text-gray-400">{history.length} records</p></div></div>
            <div className="flex items-center gap-2">
              <button onClick={() => setCsvModalOpen(true)} className="flex items-center gap-1.5 px-3 py-2 border border-gray-200 rounded-lg text-xs font-medium text-gray-600 hover:bg-gray-50 transition-colors"><FaDownload className="text-xs" /> Export CSV</button>
              <button onClick={loadAttendance} disabled={loading} className="flex items-center gap-1.5 px-3 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg text-xs font-medium transition-colors disabled:opacity-50">{loading ? <div className="w-3 h-3 border-2 border-white/30 border-t-white rounded-full animate-spin" /> : <FaChartLine className="text-xs" />} Refresh</button>
            </div>
          </div>

          {history.length === 0 && !loading ? (
            <div className="py-16 text-center"><div className="w-14 h-14 bg-gray-100 rounded-xl flex items-center justify-center mx-auto mb-4"><FaHistory className="text-gray-400 text-lg" /></div><p className="text-gray-700 font-medium">No attendance records</p><p className="text-gray-400 text-sm mt-1">Start by checking in today!</p></div>
          ) : (
                  ...
                  {history.map((record, index) => {
                    ...
                    return (
                      <tr key={record._id || index} className={`group hover:bg-gray-50 transition-colors ${isPending ? 'bg-yellow-50/30' : ''}`}>
                        ...
                        <td className="px-4 py-3 whitespace-nowrap"><span className="text-sm text-gray-800">{times.checkIn ? formatTime(times.checkIn) : '—'}</span>{record.checkInRequest?.approved === false && <span className="ml-1.5"><Badge variant="warning">Pending</Badge></span>}</td>
                        <td className="px-4 py-3 whitespace-nowrap"><span className="text-sm text-gray-800">{times.checkOut ? formatTime(times.checkOut) : '—'}</span>{record.checkOutRequest?.approved === false && <span className="ml-1.5"><Badge variant="warning">Pending</Badge></span>}</td>
                        <td className="px-4 py-3"><Badge variant={record.totalHours >= 8 ? 'success' : 'default'}>{record.totalHours?.toFixed(1) || '0.0'}h</Badge></td>
                        <td className="px-4 py-3"><Badge variant={record.status === 'present' ? 'success' : record.status === 'late' ? 'orange' : record.status === 'half-day' ? 'warning' : record.status === 'absent' ? 'danger' : 'default'}>{record.status === 'present' ? 'Present' : record.status === 'late' ? 'Late' : record.status === 'half-day' ? 'Half Day' : record.status === 'absent' ? 'Absent' : 'Pending'}</Badge></td>
                        ...
                        <td className="px-4 py-3"><div className="flex gap-2"><button onClick={() => openDetailsModal(record)} className="p-1.5 text-gray-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg transition-colors"><FaEye className="text-xs" /></button><button onClick={() => openDeleteModal(record)} className="p-1.5 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors"><FaTrash className="text-xs" /></button></div></td>
                      </tr>
                    );
                  })}
                </tbody>
