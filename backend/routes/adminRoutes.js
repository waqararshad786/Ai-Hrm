  ...
  try {
    ...
    recentLeaves.forEach(leave => {
      activities.push({
        ...
        type: 'leave',
        message: `Employee requested ${leave.type || 'leave'} leave`,
...
router.get('/dashboard/team-members', async (req, res) => {
  try {
    ...
    const teamMembers = await User.find({ isActive: true, role: { $ne: 'admin' } })
      .select('name email role department').limit(8).lean();
...
router.get('/dashboard/notifications', async (req, res) => {
  try {
    ...
    res.status(200).json({ success: true, data: { notifications, unreadCount: notifications.filter(n => !n.read).length } });
  } catch (error) {
